...
| id | status | effort | description | notes |
|---|---|---|---|---|
| REL-16 | blocked | M | Restore native live volume while preserving enlarged transcode-display video under the video-only scope. | User follow-up 3 excludes VLC patches, audio DSP, extra players and other handling. Launcher already leaves audio encoded as supplied. New CLI tests with empty audio filters, time stretching off and replay gain off still leave the private PulseAudio stream volume unchanged at RC values 256/64/128; direct playback changes it to 100/25/50 percent but VLC downsizes 1280x720 back to 320x180. `--no-sout-audio` deselects audio entirely. Sources: VLC 3.0.20 `modules/stream_out/display.c:102,150`, `src/playlist/aout.c:43-74`, `src/input/es_out.c:1751-1756`; local commands/evidence in the playback evaluation report. Resolve with a verified stock-VLC command that preserves enlarged display, audible audio and native controls, or an explicit accepted relaxation of one requirement. System mixer remains an existing workaround, not native-control restoration. |

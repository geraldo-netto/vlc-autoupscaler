...
def jobs(phase):
    if phase == "adaptive":
        return [dict(clip=clip, size=1080, zimg=12, usm=12, adaptive=adaptive,
                     frames=2048, first=0, count=0)
                for clip in ("animation", "live-action-540") for adaptive in (0, 1)]
    rows = []
...
def summarize_trace(path):
    ...
    return dict(exploration_frames=sum(row["phase"] != "3" for row in rows),
                changed_frames=[int(row["frame"]) for row in rows if row["changed"] == "1"],
                bypass_frames=sum(row["skipped"] == "1" for row in rows),
                worker_counts=sorted({int(row["workers"]) for row in rows}),
...
def capture(args, job, repeat, index):
    ...
    if not result.returncode:
        row.update(result=json.loads(result.stdout), trace_summary=summarize_trace(trace))
    return row
...
def main():
    ...
    parser.add_argument("--phase", choices=("cpu", "adaptive"), default="cpu")

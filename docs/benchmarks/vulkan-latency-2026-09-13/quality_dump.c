...
{
    const char *dump = getenv("UP_VULKAN_CPU_DUMP");
    if (dump && f->cpu_output.buf[0]) {
        FILE *file = fopen(dump, "wb");
        if (!file) abort();
        for (int plane = 0; plane < 3; plane++) {
            const plane_t *p = &f->cpu_output.pic.p[plane];
            for (int y = 0; y < p->i_visible_lines; y++)
                if (fwrite(p->p_pixels + (size_t)y * (size_t)p->i_pitch, 1,
                           (size_t)p->i_visible_pitch, file) != (size_t)p->i_visible_pitch) abort();
        }
    ...
    free(f->packed_input); free(f->packed_output);
}
...
static bool setup_gpu(scale_fixture_t *f, const char *shader, const char *usm, unsigned device)
{
    ...
    if (strcmp(f->variant, "naive") == 0) {
        ...
        up_vulkan_scale_options_t options = {.horizontal = shader, .vertical = shader,
        ...
        f->gpu = up_vulkan_create_separable(&options, device, 960, 540, f->cpu.dst_w, f->cpu.dst_h);
    }
...
{
    if (gpu) return up_vulkan_apply(f->gpu, f->packed_input, f->packed_output, f->combined ? 51 : 0) == 0;
...
static bool load_input(scale_fixture_t *f, const char *path)
{
    ...
    bool ok = fread(f->packed_input, 1, f->input_size, file) == f->input_size;
    if (fclose(file)) ok = false;
    if (!ok) return false;
    const uint8_t *input = f->packed_input;
    for (int p = 0; p < 3; p++) {
...
{
    if (argc != 6 && argc != 10 && argc != 11) return false;
...
int main(int argc, char **argv)
{
    ...
    if (!arguments(argc, argv, &device, &factor)) {
        fprintf(stderr, "usage: %s shader device factor cpu|gpu 960x540.yuv [variant usm.spv|- timing period_us [cpu_workers]]\n", argv[0]);
        return 2;
    }
    scale_fixture_t f = {0};
